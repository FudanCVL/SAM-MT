...
class TwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        attn_cross_mask: Optional[torch.Tensor] = None,
        points_per_obj: Optional[List] = None,
    ) -> Tuple[Tensor, Tensor]:
        """
        Args:
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor, attn_mask: Optional[torch.Tensor] = None, idx: Optional[int] = 0
    ) -> Tuple[Tensor, Tensor]:
        
        # Self attention block
        if self.skip_first_layer_pe:
            queries = self.self_attn(q=queries, k=queries, v=queries, attn_mask=attn_mask)
        else:
            q = queries + query_pe
...
class Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, attn_mask: Optional[torch.Tensor] = None) -> Tensor:
        ...
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).to(q.device)
