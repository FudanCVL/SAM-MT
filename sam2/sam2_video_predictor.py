...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_points_or_box(
        ...
        video_name: Optional[str] = None, # new
    ):
        ...
        current_out, _, pred_masks_gpu_sparse = self._run_single_frame_inference(
        ...
        consolidated_out = self._consolidate_temp_output_across_obj(
            ...
            obj_sparse_mask=pred_masks_gpu_sparse,
        )
        _, video_res_masks = self._get_orig_video_res_output(inference_state, consolidated_out["pred_masks_video_res"])
        _, video_res_sparse_masks = self._get_orig_video_res_output(inference_state, consolidated_out["pred_masks_video_res_sparse"])
        return frame_idx, obj_ids, video_res_masks, video_res_sparse_masks
    ...
    def add_new_mask(
        ...
        if mask.dim() != 2: 
            print("Mask should be 2D tensor, got shape:", mask.shape)
            mask = mask.squeeze()
    ...
    def _consolidate_temp_output_across_obj(
        ...
        if consolidate_at_video_res:
            ...
            consolidated_mask_key_sparse = "pred_masks_video_res_sparse"
        else:
            consolidated_H = consolidated_W = self.image_size // 4
            consolidated_mask_key = "pred_masks"

        obj_numbers = inference_state["temp_output_dict_per_obj"][0]['cond_frame_outputs'][0]['pred_masks'].shape[1]
        consolidated_out = {
            consolidated_mask_key: torch.full(
                size=(1, obj_numbers, consolidated_H, consolidated_W),
                fill_value=NO_OBJ_SCORE,
                dtype=torch.float32,
                device=inference_state["storage_device"],
            ),
            consolidated_mask_key_sparse: torch.full(
                size=(1, obj_numbers, consolidated_H, consolidated_W),
                fill_value=NO_OBJ_SCORE, 
                dtype=torch.float32,
                device=inference_state["storage_device"],
        ...
        obj_temp_output_dict = inference_state["temp_output_dict_per_obj"][0]
        ...
        if obj_mask.shape[-2:] == consolidated_pred_masks.shape[-2:]:
            ...
            resized_obj_mask = torch.nn.functional.interpolate(
                obj_mask,
                size=consolidated_pred_masks.shape[-2:], # resize to original video resolution, such as (945,1920)
                mode="bilinear",
                align_corners=False,
            )
            consolidated_out[consolidated_mask_key] = resized_obj_mask
        
        # obj_sparse_mask = out['pred_masks_sparse'] 
        consolidated_pred_sparse_masks = consolidated_out[consolidated_mask_key_sparse]
        if obj_sparse_mask.shape[-2:] == consolidated_pred_sparse_masks.shape[-2:]:
            consolidated_pred_masks[obj_idx : obj_idx + 1] = obj_mask
        else:
            resized_obj_sparse_mask = torch.nn.functional.interpolate(
                obj_sparse_mask,
                size=consolidated_pred_sparse_masks.shape[-2:], # resize to original video resolution, such as (945,1920)
                mode="bilinear",
                align_corners=False,
            )
            consolidated_out[consolidated_mask_key_sparse] = resized_obj_sparse_mask
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            pred_sparse_masks_per_obj = [None] * batch_size
            ...
            for obj_idx in range(batch_size):
                ...
                if frame_idx in obj_output_dict["cond_frame_outputs"]:
                    ...
                    pred_masks_sparse = current_out['pred_masks_sparse'].to(device, non_blocking=True)
                    ...
                    object_mem = None
                    if frame_idx == 1:
                        object_mem = obj_output_dict['cond_frame_outputs'][0]['mean_sparse_tokens_out']
                    elif frame_idx > 1:
                        object_mem = obj_output_dict['non_cond_frame_outputs'][frame_idx-1]['mean_sparse_tokens_out']
        
                    current_out, pred_masks, pred_masks_sparse = self._run_single_frame_inference(
                        inference_state=inference_state,
                ...
                pred_sparse_masks_per_obj[obj_idx] = pred_masks_sparse
            ...
            if len(pred_sparse_masks_per_obj) > 1:
                all_pred_sparse_masks = torch.cat(pred_sparse_masks_per_obj, dim=0)
            else:
                all_pred_sparse_masks = pred_sparse_masks_per_obj[0]
            _, video_res_sparse_masks = self._get_orig_video_res_output(
                inference_state
                , all_pred_sparse_masks
            )

            yield frame_idx, obj_ids, video_res_masks, video_res_sparse_masks
    ...
    def _run_single_frame_inference(
        ...
        prev_sam_mask_logits=None,
        points_per_object: Optional[List[int]] = None,
        object_mem: Optional[torch.Tensor] = None, # new
        video_name: Optional[str] = None, # new
    ):
        ...
        pred_masks_gpu_sparse = current_out["pred_masks_sparse"]
        ...
        if self.fill_hole_area > 0:
            ...
            pred_masks_gpu_sparse = fill_holes_in_mask_scores(
                pred_masks_gpu_sparse, self.fill_hole_area
            )
        
        pred_masks = pred_masks_gpu.to(storage_device, non_blocking=True)
        pred_masks_sparse = pred_masks_gpu_sparse.to(storage_device, non_blocking=True)
        ...
        compact_current_out = {
            ...
            "pred_masks": pred_masks, # unified target
            "pred_masks_sparse": pred_masks_sparse, # seperate targets
            "obj_ptr": obj_ptr,
            "object_score_logits": object_score_logits,
            "mean_sparse_tokens_out": current_out["mean_sparse_tokens_out"],
        }
        
        # New in SAM-MT: only keep the pred_masks and pred_masks_sparse for the first frame to further reduce VRAM usage
        # This is optional, and we set to False by default
        # if frame_idx > 0:
        #     compact_current_out["pred_masks"] = None
        #     compact_current_out["pred_masks_sparse"] = None
            
        return compact_current_out, pred_masks_gpu, pred_masks_gpu_sparse
